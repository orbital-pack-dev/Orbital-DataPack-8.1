...
# Обе половины double chest получают один password-id и vanilla lock.
execute as @e[tag=ms_safe_box,distance=..1.5,type=minecraft:marker] at @s run function mosseater:safe/set_password_marker

# Счётчик обнуляется: значение 1.. открыло бы окно доступа, которое немедленно
# сняло бы только что установленный замок.
scoreboard players set @e[tag=ms_safe_box,distance=..1.5,type=minecraft:marker] mosseater.safe_data 0
...
execute at @e[tag=ms_safe_box,distance=..1.5,sort=nearest,limit=1,type=minecraft:marker] align xyz run particle minecraft:happy_villager ~0.5 ~0.8 ~0.5 0.4 0.3 0.4 0.03 10 normal @a[distance=..32]
