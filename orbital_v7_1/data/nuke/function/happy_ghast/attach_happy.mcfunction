# Consume one TNT and attach one indexed display to the nearest Happy Ghast.
scoreboard players add @e[type=minecraft:happy_ghast,distance=..6,scores={ghast_tnt=..13},limit=1,sort=nearest] ghast_tnt 1
execute as @e[type=minecraft:happy_ghast,distance=..6,limit=1,sort=nearest] at @s run function nuke:happy_ghast/spawn_display
execute store result score @e[type=minecraft:happy_ghast,distance=..6,limit=1,sort=nearest] orb_ticks run data get entity @e[type=minecraft:happy_ghast,distance=..6,limit=1,sort=nearest] Health 100
