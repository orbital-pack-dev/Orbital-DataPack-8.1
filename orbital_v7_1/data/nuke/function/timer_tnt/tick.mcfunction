# Единый тик всех Таймер-ТНТ. Один проход, без per-заряд оверхеда.
...
execute as @e[type=block_display,tag=tt_display,scores={tt_timer=1..}] at @s run function nuke:timer_tnt/tick_one
