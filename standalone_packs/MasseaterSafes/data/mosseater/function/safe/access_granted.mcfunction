# Правильный ключ даёт ровно 60 тиков прямого доступа к блоку.
...
title @a[tag=ms_safe_user,limit=1] actionbar {text:"Сейф открыт на 3 секунды",color:"green"}

# Удаление выполняется последним, чтобы не потерять execution context эффектов.
