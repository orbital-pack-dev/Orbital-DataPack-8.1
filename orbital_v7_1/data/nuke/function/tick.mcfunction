...
# NukeShot upgrade: only a real rod use while NukeShot is in offhand.
execute as @a[scores={use_rod=1..}] if items entity @s weapon.offhand minecraft:fishing_rod[minecraft:custom_data~{nukeshot:1b}] at @s run function nuke:orbital_strike_cannon/upgrade_nukeshot/trigger
execute as @a[scores={use_rod=1..}] at @s run function nuke:orbital_strike_cannon/rod/handle_use_rod

# StabShot upgrade remains available in either hand.
...
# Archi-Shield: byte flag in either hand.
...
# Blast Vest: any chest item carrying the byte flag.
execute as @a[scores={bv_active=1..}] unless items entity @s armor.chest *[minecraft:custom_data~{blast_vest:1b}] run function nuke:blast_vest/reset
execute as @a if items entity @s armor.chest *[minecraft:custom_data~{blast_vest:1b}] at @s run function nuke:blast_vest/main

# Light TNT: tag new wind charges, then process from projectile context.
execute as @a at @s if items entity @s weapon.mainhand minecraft:wind_charge[minecraft:custom_data~{light_tnt:1b}] run tag @e[type=minecraft:wind_charge,distance=..8,tag=!lt_processed] add light_tnt
execute as @a at @s if items entity @s weapon.offhand minecraft:wind_charge[minecraft:custom_data~{light_tnt:1b}] run tag @e[type=minecraft:wind_charge,distance=..8,tag=!lt_processed] add light_tnt
...
# Timer TNT: only uninitialised grounded item entities enter init.
