...
# Габариты: одиночный сундук 1.1x1.4, большой сундук 2.2x1.4.
execute if block ~ ~ ~ minecraft:chest[type=single] as @e[type=minecraft:interaction,tag=ms_safe_guard,distance=..1.6] run data merge entity @s {width:1.1f,height:1.4f}
execute unless block ~ ~ ~ minecraft:chest[type=single] as @e[type=minecraft:interaction,tag=ms_safe_guard,distance=..1.6] run data merge entity @s {width:2.2f,height:1.4f}
